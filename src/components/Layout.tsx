...
export function Sidebar({ isOpen, setIsOpen }: { isOpen?: boolean, setIsOpen?: (open: boolean) => void }) {
  ...
  return (
    <div 
      className={cn(
        "fixed inset-y-0 left-0 z-50 lg:relative lg:flex h-screen bg-[#151619] text-white transition-all duration-300 flex flex-col border-r border-white/10",
        collapsed ? "lg:w-20" : "lg:w-64",
        isOpen ? "translate-x-0 w-64" : "-translate-x-full lg:translate-x-0"
      )}
    >
      <div className="p-6 flex items-center justify-between">
        {(!collapsed || isOpen) && <span className="font-bold text-xl tracking-tight italic">OmniFlow</span>}
        <button 
          onClick={() => setCollapsed(!collapsed)}
          className="hidden lg:block p-1 hover:bg-white/10 rounded-md transition-colors"
        >
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {(!collapsed || isOpen) && <span className="ml-3 font-medium">{item.label}</span>}
            </Link>
          );
        })}

        {/* Custom Modules Section */}
        {(!collapsed || isOpen) && customModules.length > 0 && (
        ...
        {customModules.map((mod) => {
          ...
          return (
            ...
            >
              ...
              {(!collapsed || isOpen) && <span className="ml-3 font-medium">{mod.name}</span>}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-white/10 space-y-1">
        ...
        >
          <Settings size={20} />
          {(!collapsed || isOpen) && <span className="ml-3 font-medium">{t('common.settings')}</span>}
        </Link>
        <button 
          onClick={logout}
        ...
        >
          <LogOut size={20} />
          {(!collapsed || isOpen) && <span className="ml-3 font-medium">{t('common.logout')}</span>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <header className="h-16 border-b border-gray-200 bg-white flex items-center justify-between px-4 lg:px-8">
      <div className="flex items-center gap-2 lg:gap-4">
        ...
        <span className="font-semibold text-gray-700 truncate max-w-[120px] lg:max-w-none">{tenant?.name || 'Carregando...'}</span>
        {tenant?.plan && (
          <span className="hidden sm:inline-block text-xs bg-gray-100 px-2 py-1 rounded text-gray-500 font-mono uppercase tracking-wider">
            {tenant.plan} Plan
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 lg:gap-6">
        <div className="hidden md:flex items-center gap-2 pr-6 border-r border-gray-200">
        ...
        <div className="flex items-center gap-3 pl-2 lg:pl-6 border-l border-gray-200">
          ...
          <div className="h-8 w-8 lg:h-10 lg:w-10 rounded-full bg-gray-200 overflow-hidden shrink-0">
            <img 
